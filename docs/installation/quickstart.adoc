...
Network-Enforcer does not observe traffic on its own: it consumes flow information from the data plane
...
The `values.cni.ambient.ipv6=false` flag is not required by Network-Enforcer, but `istio-cni` programs
...
Goldmane deployment, and then creates the `net-enf-goldmane-client-certs` Secret that Network-Enforcer
uses as a client certificate when it scrapes flows from Goldmane.

[IMPORTANT]
====
Run the script *before* installing the Network-Enforcer chart.
...
The script installs Cilium with the settings Network-Enforcer needs, and waits for the agent, the
...
Network-Enforcer then keeps working — learning, monitor and protect are unaffected — but violations are
...
Network-Enforcer observes the connections in the cluster and, for each endpoint of a connection, creates
...
Both providers report full flows, so Network-Enforcer creates an egress proposal for the client and an
